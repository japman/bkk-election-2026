...
**Architecture:** Edge-first — Cloudflare cache HTML `/`+`/council` ~5วิ (origin render ~1/5วิ ไม่ว่ากี่พันคน), หน้า สก อัปเดตเองด้วยการ poll snapshot JSON จาก CloudFront (ภาระ origin 0), และ governor WebSocket เปิด/ปิดสดผ่านคอลัมน์ `live_streaming` บน Election (toggle จากหน้า admin แบบเดียวกับ `data_mode`)
...
- Kill-switch เก็บเป็นคอลัมน์ boolean **`live_streaming` บน Election** (`default: true`); toggle จาก admin แบบเดียวกับ `data_mode`/`toggle_mode` ที่มีอยู่ (reuse `Admin::ElectionsController`); flip ถึง visitor ใหม่ภายใน ~5วิ (edge TTL) — ไม่ purge CF
- Cloudflare Cache Rule = **ตั้งเองใน dashboard (manual)**, ไม่ใช้ CF API
- การ broadcast governor (turbo_stream) ต้องห่อด้วย `@election.live_streaming?`; เมื่อปิด → fallback poller (มีอยู่) poll ทันที (`data-fallback-stale-after-value="0"`)
- ทุก task: ของเดิมต้อง green — รัน `bundle exec rspec` ผ่านก่อน commit

**ลำดับ & ความอิสระ:** Task 1 (A) → Task 2,3 (B) → Task 4 (C) → Task 5 (runbook). แต่ละ component (A/B/C) ship แยกได้ อิสระต่อกัน
...
### Task 4: Admin live-WS kill-switch (`Election#live_streaming` column + toggle + gate)

เพิ่มคอลัมน์ boolean บน Election + ปุ่ม admin toggle (mirror `toggle_mode` ที่มีอยู่) + gate `turbo_stream_from` บนหน้า governor. ปิดแล้ว → fallback poller (มีอยู่) poll ทันที

**Files:**
- Create: `db/migrate/<timestamp>_add_live_streaming_to_elections.rb` (ผ่าน generator)
- Modify: `config/routes.rb` (เพิ่ม `patch :toggle_streaming` ใน `resource :election`)
- Modify: `app/controllers/admin/elections_controller.rb` (เพิ่ม `#toggle_streaming`)
- Modify: `app/views/admin/dashboard/index.html.erb` (ปุ่ม toggle)
- Modify: `app/views/dashboard/show.html.erb` (gate stream)
- Test: `spec/requests/dashboard_spec.rb` (เพิ่ม), `spec/requests/admin_spec.rb` (เพิ่ม)

**Interfaces:**
- Produces: column `elections.live_streaming` (boolean, default true) → `Election#live_streaming?`; route helper `toggle_streaming_admin_election_path` (PATCH)
- Consumes: `Election.current` (มีอยู่); fallback controller value `data-fallback-stale-after-value` (Stimulus "fallback", value "staleAfter"); existing `toggle_mode` action ใน `Admin::ElectionsController` (mirror)

- [ ] **Step 1: Generate + edit migration**

Run: `bin/rails generate migration AddLiveStreamingToElections live_streaming:boolean`
แล้วแก้ไฟล์ `db/migrate/<timestamp>_add_live_streaming_to_elections.rb` ให้เป็น:
```ruby
...
Expected: คอลัมน์ `live_streaming` (default true) ใน `elections`; `db/schema.rb` อัปเดต (rows เดิมได้ default true)

- [ ] **Step 3: Write failing dashboard gate tests**
...
Expected: FAIL — stream อยู่เสมอ และไม่มี `data-fallback-stale-after-value="0"`

- [ ] **Step 5: Implement governor view gate**
...
<% else %>
  <% if @election.live_streaming? %>
    <%= turbo_stream_from "results" %>
  <% end %>
  <div data-controller="live-flash fallback"
       data-fallback-stale-after-value="<%= @election.live_streaming? ? 15000 : 0 %>">
...
- [ ] **Step 6: Run to verify dashboard specs pass**

Run: `bundle exec rspec spec/requests/dashboard_spec.rb`
Expected: PASS (รวมของเดิม)

- [ ] **Step 7: Write failing admin toggle test**
...
Expected: FAIL — `undefined local variable or method 'toggle_streaming_admin_election_path'`

- [ ] **Step 9: Implement route**

`config/routes.rb` แก้บล็อก `resource :election` ใน `namespace :admin` ให้เป็น:
```ruby
...
- [ ] **Step 10: Implement controller action**

เพิ่ม method ใหม่ใน `app/controllers/admin/elections_controller.rb`:
```ruby
...
- [ ] **Step 11: Run to verify admin toggle passes**

Run: `bundle exec rspec spec/requests/admin_spec.rb -e "toggles live streaming"`
Expected: PASS

- [ ] **Step 12: Implement admin toggle button**
...
```erb
  <section class="admin-card">
    <div class="mode-line">
      ...
      <span class="mode-pill <%= @election.live_streaming? ? "api" : "manual" %>">
        <span class="dot"></span>
        <%= @election.live_streaming? ? "ON — push สด (ปกติ)" : "OFF — โหมด peak (ทุกคน poll CDN)" %>
      </span>
      <%= button_to @election.live_streaming? ? "ปิด WS (โหมด peak)" : "เปิด WS",
            toggle_streaming_admin_election_path, method: :patch,
            class: "btn #{@election.live_streaming? ? 'btn-ghost' : 'btn-primary'} btn-sm",
            data: { turbo_confirm: "ยืนยันสลับโหมด Live WS? มีผลกับ visitor ใหม่ภายใน ~5 วิ" } %>
    </div>
  </section>
```

- [ ] **Step 13: Run full suite (regression)**
...
```bash
git add db/migrate db/schema.rb config/routes.rb app/controllers/admin/elections_controller.rb app/views/admin/dashboard/index.html.erb app/views/dashboard/show.html.erb spec/requests/dashboard_spec.rb spec/requests/admin_spec.rb
git commit -m "Admin live-WS kill-switch via Election#live_streaming column + governor stream gate"
```

---

### Task 5: Election-day runbook (เอกสาร, ไม่มีโค้ด)
...
- **Spec coverage:** A→Task 1, B→Task 2-3, C→Task 4, D→Task 5 ครบทุก §
- **Placeholder scan:** ไม่มี TBD; โค้ดเต็มทุก step (`<timestamp>` ใน migration = generator เติมให้)
- **Type consistency:** `council_seat_breakdown` คืน `{party,color,seats}` ใช้ตรงกันใน partial/snapshot/JS; `@election.live_streaming?` ใช้ตรงกันใน view/admin/controller; `toggle_streaming_admin_election_path` (PATCH) ตรงกับ route `resource :election { patch :toggle_streaming }`
- **karpathy review:** ใช้คอลัมน์บน Election (mirror `toggle_mode` ที่มีอยู่) แทน generic `Setting` model — surgical, consistent, โค้ดน้อยลง
